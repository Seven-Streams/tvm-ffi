...
impl IntPair {
    ...
    pub fn scale(&mut self, _0: i64) -> Result<()> {
        thread_local!(static F: std::cell::OnceCell<tvm_ffi::Function> = const { std::cell::OnceCell::new() });
        let f = get_type_method_cached(&F, IntPairObj::type_index(), "scale")?;
        Ok(f.call_packed(&[AnyView::from(&*self), AnyView::from(&_0)])?.try_into()?)
    }

    pub fn swapped(&mut self) -> Result<IntPair> {
        thread_local!(static F: std::cell::OnceCell<tvm_ffi::Function> = const { std::cell::OnceCell::new() });
        let f = get_type_method_cached(&F, IntPairObj::type_index(), "swapped")?;
        Ok(f.call_packed(&[AnyView::from(&*self)])?.try_into()?)
    }

    pub fn checked_div(&mut self, _0: i64) -> Result<i64> {
        thread_local!(static F: std::cell::OnceCell<tvm_ffi::Function> = const { std::cell::OnceCell::new() });
        let f = get_type_method_cached(&F, IntPairObj::type_index(), "checked_div")?;
        Ok(f.call_packed(&[AnyView::from(&*self), AnyView::from(&_0)])?.try_into()?)
    }
}
// tvm-ffi-stubgen(end)
