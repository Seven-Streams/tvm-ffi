...
# Refresh the generated tree in place. With --init-* any missing scaffolding
# (files/markers) is recreated; existing directive blocks -- and hand-added
# content outside them, such as the license headers -- are preserved. The
# --init-* values mirror the CMake STUB_* options in CMakeLists.txt.
# Note: step 1's cmake build already ran stubgen as a POST_BUILD step; this
# step demonstrates the equivalent CLI invocation (an idempotent re-run).
