...
class UnsupportedTypeError(Exception):
    ...
    ``origin`` is the offending FFI type origin (e.g. ``"Map"``) or identifier;
    ``reason`` overrides the default message. ``cli`` catches this around object
    block generation and skips the block with a ``[Skipped]`` warning instead of
