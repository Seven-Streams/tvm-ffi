...
Turning this metadata into source text is the job of a backend's code
generator (e.g. :mod:`tvm_ffi.stub.python_backend.codegen`).
"""
...
def _python_codegen_module() -> Any:
    """Lazily import Python codegen helpers to avoid import cycles."""
    return importlib.import_module("tvm_ffi.stub.python_backend.codegen")


# Backward-compatible re-export used by existing tests/callers.
ImportItem = importlib.import_module("tvm_ffi.stub.python_backend.imports").ImportItem
...
class Options:
    ...
    dry_run: bool = False
    target: str = "python"
    """Code-generation backend to use, e.g. ``"python"`` or ``"rust"``."""
...
class FuncInfo:
    ...
    def gen(self, ty_map: Callable[[str], str], indent: int) -> str:
        """Backward-compatible wrapper for the Python backend signature renderer."""
        return _python_codegen_module().render_func_signature(self, ty_map, indent)
...
class ObjectInfo:
    ...
    def gen_fields(self, ty_map: Callable[[str], str], indent: int) -> list[str]:
        """Backward-compatible wrapper for Python field rendering."""
        return _python_codegen_module().render_object_fields(self, ty_map, indent)

    def gen_methods(self, ty_map: Callable[[str], str], indent: int) -> list[str]:
        """Backward-compatible wrapper for Python method rendering."""
        return _python_codegen_module().render_object_methods(self, ty_map, indent)

    def gen_ffi_init(self, ty_map: Callable[[str], str], indent: int) -> list[str]:
        """Backward-compatible wrapper for Python ``__ffi_init__`` rendering."""
        return _python_codegen_module().render_object_ffi_init(self, ty_map, indent)

    def gen_init(self, ty_map: Callable[[str], str], indent: int) -> list[str]:
        """Backward-compatible wrapper for Python ``__init__`` rendering."""
        return _python_codegen_module().render_object_init(self, ty_map, indent)
