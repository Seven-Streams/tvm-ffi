...
# under the License.
"""The Python code-generation backend for ``tvm-ffi-stubgen``.

:class:`PythonBackend` implements the :class:`tvm_ffi.stub.backend.Backend`
protocol by delegating to :mod:`.codegen`. It owns the Python notion of an
import (:class:`.imports.ImportItem` / :class:`.imports.PythonImports`); the
...
class PythonBackend:
    ...
    def render_type(self, schema: TypeSchema, ty_render: Callable[[str], str]) -> str:
    ...
    def generate_global_funcs_block(
        ...
        G.generate_python_global_funcs(
            code, global_funcs, ty_map, imports.items, opt, self.render_type
        )
    ...
    def generate_object_block(
        ...
        G.generate_python_object(code, ty_map, imports.items, opt, obj_info, self.render_type)
