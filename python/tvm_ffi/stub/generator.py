...
#: requires. Generators build one of these per block.
...
def get_generator(target: str) -> Generator:
    ...
    try:
        factory = _GENERATORS[target]
    except KeyError as e:
        known = ", ".join(sorted(_GENERATORS))
        raise ValueError(f"Unknown stubgen generator: {target!r}. Known generators: {known}") from e
    return factory()
