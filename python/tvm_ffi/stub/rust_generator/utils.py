...
def _use(imports: RustImports, path: str, alias: str | None = None) -> None:
    """Record a boilerplate ``use`` (deduped, alias-aware) onto the collector.

    Boilerplate ``use``s are recorded before any field/method types are rendered,
    so they claim their (un-aliased) leaf names first; a later type whose leaf
    collides is the one that gets aliased.
    """
    imports.record(path, alias=alias)
