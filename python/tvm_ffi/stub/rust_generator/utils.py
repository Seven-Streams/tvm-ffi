...
class RustUse:
    ...
    ``Option``) stay bare and need no ``use``.
    """
...
class RustImports:
    ...
    def record(self, name: str, alias: str | None = None) -> str:
        ...
        Bare prelude/primitive names record no ``use``; paths in
        :data:`~.consts.RUST_NO_IMPORT_FULLPATH` render fully-qualified inline
        instead, to avoid shadowing a prelude name (e.g. ``String``).
        """
...
def render_rust_type(schema: TypeSchema, ty_render: Callable[[str], str]) -> str:
    ...
    if origin == "Optional":
        assert args  # TypeSchema's post_init guarantees the arg.
