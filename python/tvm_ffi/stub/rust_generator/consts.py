...
#: Default FFI-origin -> Rust-type map. Values are fully qualified paths so the
#: ``TyRenderer`` can derive both the leaf name and the ``use`` import; values
#: without ``::`` (primitives, prelude ``Option``) need no import.
...
#: Paths rendered fully qualified inline, never via ``use``: importing
#: ``tvm_ffi::String`` would shadow the prelude ``String`` module-wide and break
#: ``#[derive(ObjectRef)]``'s generated ``fn type_str() -> String``.
RUST_NO_IMPORT_FULLPATH = frozenset({"tvm_ffi::String"})

#: Origins whose Rust render type is not memory-identical to the C++ field type
#: (std ``Option<T>`` != ``ffi::Optional<T>``, Rust tuple != ``ffi::Tuple``): a
#: native struct-literal write would store the wrong bytes, so a type with such
#: a top-level field falls back to FFI construction. Nested uses (e.g.
#: ``Array<Optional<T>>``) live in the container's FFI-encoded heap and are safe.
RUST_NATIVE_UNSAFE_ORIGINS = frozenset({"Optional", "tuple"})

#: Origins the crate has no FFI type for (do NOT map to ``HashMap``/``Vec``);
#: ``render_type`` raises ``UnsupportedTypeError`` and the enclosing object is
#: skipped.
