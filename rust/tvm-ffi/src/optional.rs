...
impl<T, A, const N: usize> Optional<T, A, N> {
    ...
    pub unsafe fn read_with_getter<V: AnyCompatible>(
        &self,
    ...
    pub unsafe fn write_with_setter<V: AnyCompatible>(
        &self,
...
const FIELD_FLAG_WRITABLE: i64 = 1 << 0;
...
pub struct FieldAccess {
    ...
    /// Field byte offset within the object (authoritative; for layout asserts).
    pub offset: i64,
    /// Field byte size.
    pub size: i64,
    /// Field alignment.
    pub alignment: i64,
    /// Whether the field is writable (vs. read-only/serialization-only).
    pub writable: bool,
}
    ...
    unsafe {
        ...
        for f in fields {
            ...
            return Ok(FieldAccess {
                ...
                writable: (f.flags & FIELD_FLAG_WRITABLE) != 0,
            });
        }
        Err(Error::new(
            TYPE_ERROR,
...
/// Compile-time assertion that an [`Optional`] mirror matches the size/alignment
/// reported by the C++ reflection registry. Intended for stubgen-generated code.
///
/// ```ignore
/// assert_optional_layout!(Optional<i64, Align8, 16>, size = 16, align = 8);
/// ```
#[macro_export]
macro_rules! assert_optional_layout {
    ($ty:ty, size = $size:expr, align = $align:expr) => {
        const _: () = {
            assert!(
                ::core::mem::size_of::<$ty>() == $size,
                "ffi::Optional mirror size mismatch with C++ reflection registry"
            );
            assert!(
                ::core::mem::align_of::<$ty>() == $align,
                "ffi::Optional mirror alignment mismatch with C++ reflection registry"
            );
        };
    };
}
...
mod tests {
    ...
    // Compile-time guard (the form stubgen emits per optional field).
    crate::assert_optional_layout!(Optional<i64, Align8, 16>, size = 16, align = 8);
    crate::assert_optional_layout!(Optional<u16, Align2, 6>, size = 6, align = 2);

    // Pure-Rust layout checks: every C++ Optional category, by (marker, N), must
    // reproduce the size/align measured against the real headers (see plan.md).
    #[test]
    fn layout_matches_cpp_categories() {
        ...
        // Category C (std::optional<scalar> fallback)
        check!(i64, Align8, 16, 16, 8);
        check!(f64, Align8, 16, 16, 8);
        check!(i32, Align4, 8, 8, 4);
        check!(u8, Align1, 2, 2, 1); // Optional<bool>
        check!(u32, Align4, 12, 12, 4); // Optional<DLDevice>
        check!(u16, Align2, 6, 6, 2); // Optional<DLDataType>
                                      // Category B (String / Bytes)
        check!(crate::String, Align8, 16, 16, 8);
        // Category A (ObjectRef pointer)
        check!(*mut c_void, Align8, 8, 8, 8);
    }
    ...
    // `Send` follows `T`; `UnsafeCell` makes it unconditionally `!Sync`.
    #[test]
    fn auto_traits() {
        fn assert_send<T: Send>() {}
        assert_send::<Optional<i64, Align8, 16>>();
        // (!Sync is enforced structurally by UnsafeCell and verified by review.)
    }
}
